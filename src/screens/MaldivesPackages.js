...
// This stripHtmlTags function can be removed if all HTML content is rendered by RenderHtml.
// For now, I'll keep it as other parts of your code might still use it for titles etc.
function stripHtmlTags(html) {
  return html?.replace(/<[^>]*>?/gm, '') || '';
}

// REMOVE THE OLD renderHtmlContent function as it will be replaced by RenderHtml component
/*
const renderHtmlContent = (htmlContent) => {
  if (!htmlContent) return null;
...
export default function MaldivesPackages({ navigation, route }) {
  ...
  // Define the base styles for your HTML content to be used with RenderHtml
  const htmlRenderTagsStyles = {
    h1: { fontSize: 24, fontWeight: 'bold', color: colors.darkGray, marginBottom: 10, textAlign: 'center' },
    h2: { fontSize: 20, fontWeight: 'bold', color: colors.darkGray, marginBottom: 8 },
    h3: { fontSize: 18, fontWeight: 'bold', color: colors.darkGray, marginBottom: 6 },
  ...
  return (
    <View style={styles.container}>
      <Header title="Maldives Packages" showNotification={true} navigation={navigation} />
      <ScrollView
        contentContainerStyle={styles.scrollContainer}
        showsVerticalScrollIndicator={false}>
        {/* Maldives Slider Section (Replaces the single banner) */}
                ...
                {isDataLoaded && (
                  <View style={styles.scrollableDescriptionWrapper}>
                    <ScrollView
                      ...
                      onScroll={(e) => setScrollPosition(e.nativeEvent.contentOffset.y)}
                      scrollEventThrottle={16}>
                      {/* Use RenderHtml for top_desc */}
                      {singleDestination?.data?.top_desc && (
                        <RenderHtml
                          contentWidth={width - (2 * 15 + 2 * 10)} // Adjust based on sectionWithSearchMarginSafari and customCardContainer padding
                          source={{ html: singleDestination.data.top_desc }}
                          tagsStyles={htmlRenderTagsStyles}
                          enableExperimentalCSSPadding
                          enableExperimentalMarginCollapsing
                          enableExperimentalContentCssHeight
                          renderersProps={{
                            a: {
                              onPress: (event, href) => {
                                Linking.openURL(href); // Handle links
                              },
                            },
                          }}
                        />
                       ...
                       {singleDestination?.data?.top_head && (
                         <RenderHtml
                           contentWidth={width - (2 * 15 + 2 * 10)} // Same content width
          ...
          {isDataLoaded && (
            <View style={styles.scrollableDescriptionWrapper}>
              <ScrollView
                ...
                onScroll={(e) => setScrollPosition(e.nativeEvent.contentOffset.y)}
                scrollEventThrottle={16}>
                {/* Use RenderHtml for things_todo_content */}
                {singleDestination?.data?.things_todo_content && (
                  <RenderHtml
                    contentWidth={width - (2 * 15 + 2 * 10)} // Adjust content width for padding
                    source={{ html: singleDestination.data.things_todo_content }}
                    tagsStyles={htmlRenderTagsStyles}
                    enableExperimentalCSSPadding
                    enableExperimentalMarginCollapsing
                    enableExperimentalContentCssHeight
...
const styles = StyleSheet.create({
  ...
  customCardContainer: {
    backgroundColor: colors.white,
    borderRadius: 12,
    padding: 10, // Added padding for content inside the card
    marginVertical: 10,
    shadowColor: colors.black,
    shadowOpacity: 0.05,
    shadowRadius: 8,
  ...
  cardMulti: {
    width: MULTI_CENTER_CARD_WIDTH,
    // Removed marginBottom and marginHorizontal from here, handled by columnWrapperStyle and conte
  },

  // Add styles for the HTML content if needed, these correspond to the `tagsStyles`
  contentHeading2: {
    fontSize: 20,
    fontWeight: 'bold',
    color: colors.darkGray,
  ...
  contentLink: {
    color: colors.primary, // Or your specific link color
  ...
  // Styles already existing in your code
  card: {
    width: MULTI_CENTER_CARD_WIDTH,
    height: MULTI_CENTER_CARD_HEIGHT,
    borderRadius: 10,
  ...
  sliderSection: {
    paddingVertical: 20,
    // paddingHorizontal: 15, // Content inside FlatList will handle padding
  },
  headingPlaces: {
    fontSize: 20,
    fontWeight: 'bold',
    ...
    textAlign: 'center',
    marginBottom: 15,
    paddingHorizontal: 15, // To match general section padding
  },
  horizontalSliderContent: {
    paddingHorizontal: 15, // Space at the start and end of the FlatList
  },
  slideItemThings: {
    width: width - 40, // Card width, considering 20px padding on each side
    marginRight: 10,
    backgroundColor: colors.white,
    borderRadius: 10,
    overflow: 'hidden',
  ...
  leftArrowThingsToDo: {
    ...
    left: 0,
    top: '50%',
    transform: [{ translateY: -12.5 }], // Center vertically
    backgroundColor: colors.white,
    borderRadius: 25,
    padding: 10,
    shadowColor: colors.black,
  ...
  leftArrowFoods: { // New style for foods slider arrows
    position: 'absolute',
    left: 0,
    top: '50%',
    transform: [{ translateY: -12.5 }], // Center vertically
  ...
  rightArrowFoods: { // New style for foods slider arrows
    position: 'absolute',
    right: 0,
    top: '50%',
    transform: [{ translateY: -12.5 }], // Center vertically
  ...
  famousPlacesContentContainer: {
    paddingHorizontal: ITEM_SPACING, // Ensures first and last item are centered
  },
  cardPlaces: {
    height: 250,
    borderRadius: 10,
