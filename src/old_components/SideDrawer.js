...
import {
  ...
  ImageBackground,
  Linking,
  ActivityIndicator, // Although not directly used in this version, kept for completeness
  FlatList,
} from 'react-native';
import { Drawer } from 'react-native-drawer-layout'; // Import Drawer component
import { useDispatch, useSelector } from 'react-redux';
import {
  selectAllDestinations,
  allDestinationsStatus,
...
import CrossIcon from '../assets/images/cross.png';
 import Goldenarrow from '../assets/images/GoldenArrrow.svg';
 import BackGoldenArrow from '../assets/images/BackGoldenArrow.svg';
// Assuming these are local assets, ensure their paths are correct
// import InstagramIcon from '../assets/images/insta.png';
// import FacebookIcon from '../assets/images/fb.png';
// import PinterestIcon from '../assets/images/pinteret.png';
// import YouTubeIcon from '../assets/images/youtube.png';
// import TwitterIcon from '../assets/images/twitter.png';
//  // Assuming these are SVG components
 // Assuming these are SVG components
import Logo from '../assets/images/Logo.png';
const { width, height } = Dimensions.get('window');
// const ICONS_TO_DISPLAY = [
...
export default function SideDrawer({ isOpen, onClose, navigation }) {
  ...
  const country = useSelector(selectCountryDestinations);
  const allDestinations = useSelector(selectAllDestinations);
  const destinationsStatus = useSelector(allDestinationsStatus); // Kept for potential loading indicators
  const [activeItem, setActiveItem] = useState(null);
  const [activeName, setActiveName] = useState('');
  const [city, setCity] = useState(null);
  // const [isSubLoading, setIsSubLoading] = useState(false); // Not used in this version
  // const socialLinks = useMemo(
  //   () =>
  //     ICONS_TO_DISPLAY.map((item) => ({
  //       ...item,
  ...
  const handleMenuItemPress = useCallback(
    (item, single = false) => {
      if (item.name === 'Destinations' && !single) {
        ...
        onClose();
        if (item.screen) navigation.navigate(item.screen);
        setActiveItem(null); // Reset active item when navigating
        setCity(null); // Reset city when navigating
        setActiveName(''); // Reset active name when navigating
      }
    },
    [navigation, onClose]
  );
  ...
  return (
    <Drawer
      open={isOpen} // Control drawer visibility with isOpen prop
      onOpen={() => { /* This is called when the drawer starts opening, isOpen is already true */ }}
      onClose={onClose} // Use the provided onClose prop
      drawerPosition="left" // Assuming you want it from the left
      drawerStyle={styles.drawerBody} // Apply your existing drawerBody styles for width and background
      renderDrawerContent={() => (
        // Your existing FlatList and header/footer logic
        <View style={{ flex: 1, backgroundColor: '#ffffff' }}> {/* Inner container for drawer content */}
          {renderHeader()}
          <FlatList
            data={getData}
            keyExtractor={(item, index) => item.id?.toString() || item.name || String(index)}
      ...
      // The content *behind* the drawer is where this component will be rendered.
      // Since this component is a standalone drawer, we don't need to render anything here.
      // In a full app, this would typically be your main screen content.
    >
      {/* If this SideDrawer component is used as a standalone element that appears over other content
          (e.g., within a larger `App` component that also contains your main screen),
          you might render a transparent view here or nothing if it truly only acts as the drawer itself.
          For now, keeping it empty as the `renderDrawerContent` handles the actual UI. */}
