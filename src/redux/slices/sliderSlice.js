...
export const fetchCruiseSliders = createAsyncThunk(
  'slider/fetchCruiseSliders',
  async (_, thunkAPI) => {
    try {
      const res = await api.get('sliders?type=cruise');
     
      return res.data;
    } catch (err) {
      console.log('Cruise Sliders Error:', err.message);
      return thunkAPI.rejectWithValue(err.message);
    }
  }
);
...
extraReducers: (builder) => {
    ...
    builder
      ...
      builder.addCase(fetchCruiseSliders.pending, (state) => {
        state.loading = true;
        state.status = 'loading';
      })
      .addCase(fetchCruiseSliders.fulfilled, (state, action) => {
        state.cruiseSliders = action.payload.data || [];
        state.loading = false;
        state.status = 'succeeded';
      })
      .addCase(fetchCruiseSliders.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
        state.status = 'failed';
      });
...
export const selectMaldivesSliders = (state) => state.slider.maldivesSliders;
export const selectSpecialOffers = (state) => state.slider.specialOffesSliders;
export const selectCruiseSliders = (state) => state.slider.cruiseSliders;
