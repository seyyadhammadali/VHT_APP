...
import { selectCountryDestinations } from '../redux/slices/destinationsSlice';
...
export default function TopDestination({ navigation }) {
  console.log("page render", new Date().toLocaleTimeString());
  
  const single = useSelector(selectFilteredPage('destinations'));
  const destinations = useSelector(selectCountryDestinations);
  const destination_status = destinations?'':'loading';
  const loading = false;
...
useEffect(() => {
    ...
    (destinationId) => {
      navigation.navigate('MaldivesPackages', { destinationId });
    },
    [navigation]
  );
const DestinationCard = memo(({ item, onPress }) => {
  return (
    <TouchableOpacity style={styles.card} onPress={() => onPress(item.id)}>
      <ImageBackground
        source={{ uri: item?.banne }}
  ...
  const renderDestinationCard = useCallback(
    ({ item }) => <DestinationCard item={item} onPress={handleNavigate} />,
    [handleNavigate]
  );
